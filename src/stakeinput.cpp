...
// The zSPD block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSPD is a hash of the serial
...
{
    //Create an output returning the zSPD that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSPDOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSPD output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSPD", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSPDOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSPD output", __func__);
...
{
    CzSPDTracker* zspdTracker = pwallet->zspdTracker.get();
    CMintMeta meta;
    if (!zspdTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zspdTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SPD stake is the outpoint
