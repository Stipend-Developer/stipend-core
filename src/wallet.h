...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 SPD at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSPD send
enum ZerocoinSpendStatus {
    ZSPD_SPEND_OKAY = 0,                            // No error
    ZSPD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSPD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSPD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSPD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSPD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSPD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSPD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSPD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSPD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSPD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSPD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSPD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSPD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSPD_SPENT_USED_ZSPD = 14,                      // Coin has already been spend
    ZSPD_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSPD_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSPDOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzspdAuto) const;
    ...
    */
    ...
    CzSPDWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSPDTracker> zspdTracker;
    ...
    void setZWallet(CzSPDWallet* zwallet)
    {
        zwalletMain = zwallet;
        zspdTracker = std::unique_ptr<CzSPDTracker>(new CzSPDTracker(strWalletFile));
    }

    CzSPDWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSPD reset */
    boost::signals2::signal<void()> NotifyzSPDReset;
