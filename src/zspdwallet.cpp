...
CzSPDWallet::CzSPDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zspd seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSPDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSPDSeed_deprecated()) {
                    LogPrintf("%s: Updated zSPD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zspd seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zspd wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSPDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSPDCount(nCountLastUsed);
    else if (!walletdb.ReadZSPDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSPDWallet::Lock()
{
    seedMaster = 0;
}

void CzSPDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSPDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSPD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSPDWallet::LoadMintPoolFromDB()
{
...
void CzSPDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSPDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSPDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zspdTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSPDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSPD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zspdTracker which also adds to database
    pwalletMain->zspdTracker->Add(dMint, true);
...
void CzSPDWallet::SeedToZSPD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSPDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSPDWallet::GenerateDeterministicZSPD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSPDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSPDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
