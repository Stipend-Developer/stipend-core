...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"stipendprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"stipendprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Stipend address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"stipendaddress\"\n"
            "\nReveals the private key corresponding to 'stipendaddress'.\n"
            "Then the importprivkey can be used with this output\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"stipendaddress\"   (string, required) The Stipend address for the private key\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Stipend %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"stipendaddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'stipendaddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"stipendaddress\"   (string, required) The Stipend address for the private key (you must hold the key already)\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("bip38encrypt", "\"sRJGzdWvQJWJpdvWrt6f1UUBpBgAXKRuv8\" \"mypasphrase\"") +
            HelpExampleRpc("bip38encrypt", "\"sRJGzdWvQJWJpdvWrt6f1UUBpBgAXKRuv8\" \"mypasphrase\""));
...
UniValue bip38decrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"stipendaddress\" \"passphrase\"\n"
