...
// Copyright (c) 2016-2018 The Stipend developers
...
public:
    ...
    bool WriteZSPDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSPDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSPDSeed_deprecated(uint256& seed);
    bool EraseZSPDSeed();
    bool EraseZSPDSeed_deprecated();

    bool WriteZSPDCount(const uint32_t& nCount);
    bool ReadZSPDCount(uint32_t& nCount);
