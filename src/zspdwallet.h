...
#ifndef STIPEND_ZSPDWALLET_H
#define STIPEND_ZSPDWALLET_H
...
public:
    CzSPDWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSPD(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSPD(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //STIPEND_ZSPDWALLET_H
